...
void RTC_NVIC(RTC_NVIC_config type) {
    switch(type) {
        case RTC_ENABLE_WAKEUP:
            exe()->write_bit_block(NVIC->ISER, 1, RTC_WKUP_IRQn, 1);
            break;
        case RTC_ENABLE_ALARM:
            exe()->write_bit_block(NVIC->ISER, 1, RTC_Alarm_IRQn, 1);
            break;
        case RTC_DISABLE_WAKEUP:
            exe()->write_bit_block(NVIC->ICER, 1, RTC_WKUP_IRQn, 1);
            break;
        case RTC_DISABLE_ALARM:
            exe()->write_bit_block(NVIC->ICER, 1, RTC_Alarm_IRQn, 1);
            break;
        default:
...
void RTC_Bkp_write(uint8_t registerIndex, uint8_t data) {
    ...
    exe()->write_bit_block(&RTC->BKP0R, BYTE_BITS, (registerIndex * BYTE_BITS), data);
...
uint8_t RTC_Bkp_read(uint8_t registerIndex) {
    ...
    if (registerIndex < MAX_BACKUP_REGISTERS) {
        value = exe()->get_bit_block(&RTC->BKP0R, BYTE_BITS, (registerIndex * BYTE_BITS));
    }
...
void RTC_PWR_clock(uint8_t state)
{
	exe()->write_block(&RCC->APB1ENR, 1, RCC_APB1ENR_PWREN_Pos, state); // Power interface clock enable
}
void RTC_BkpSram_clock(uint8_t state)
{
	#ifdef STM32F446xx
		exe()->write_block(&RCC->AHB1ENR, 1, RCC_AHB1ENR_BKPSRAMEN_Pos, state); // Backup SRAM interface clock enable
	#endif
	exe()->write_block(&RCC->AHB1LPENR, 1, RCC_AHB1LPENR_SRAM1LPEN_Pos, state); // Backup SRAM interface clock enable
}
void RTC_Write_enable(void)
{
...
static void set_rtc_clock_source(uint8_t clock_source) {
    RTC_Write_enable();
    exe()->write_field(&RCC->BDCR, RCC_BDCR_RTCSEL_Msk, RCC_BDCR_RTCSEL_Pos,clock_source);
    RTC_Write_disable();
}
void RTC_L_select(uint8_t lclock) {
    // Input validation
    ...
    RTC_Write_enable();
    exe()->write_field(&RCC->BDCR, RCC_BDCR_RTCSEL_Msk, RCC_BDCR_RTCSEL_Pos,0);
    RTC_Write_disable();
