...
{
	exe()->write_bit_block(NVIC->ISER, 1, IRQn, 1);
}
void NVIC_clear_enable( uint8_t IRQn )
{
	exe()->write_bit_block(NVIC->ICER, 1, IRQn, 1);
}
...
{
	exe()->write_bit_block(NVIC->ISPR, 1, IRQn, 1);
}
void NVIC_clear_pending( uint8_t IRQn )
{
	exe()->write_bit_block(NVIC->ICPR, 1, IRQn, 1);
}
uint8_t NVIC_active( uint8_t IRQn ) // Query
{
	...
	//return nvic_getexe()->write_bit_block(NVIC->ICPR, 1, IRQn);
	return state;
}
...
{
	exe()->write_block(&NVIC->STIR, 9, 0, IRQn);
}
