...
void STM32FXXX_Rcc_Pwr_Clock(uint8_t state)
{
	exe()->write_block(&dev()->rcc->APB1ENR, 1, RCC_APB1ENR_PWREN_Pos, state); // Power interface clock enable
}
void STM32FXXX_Rcc_Write_Enable(void)
{
    ...
    while(rdy) {
        switch(choice) {
            case RCC_CLK_HSI: // HSION: Internal high-speed clock enable
                if(set) {
                	exe()->write_encoded(&dev()->rcc->CR, RCC_CR_HSION_Msk, RCC_CR_HSION); // Enable HSI
                    timeout = 1000;
                    set = 0;
                }
            ...
            case RCC_CLK_HSE: // HSEON: External high-speed clock enable
                if(set) {
                	exe()->write_encoded(&dev()->rcc->CR, RCC_CR_HSEON_Msk, RCC_CR_HSEON); // Enable HSE
                    timeout = 0x1FFFFF;
                    set = 0;
                }
                else if(dev()->rcc->CR & RCC_CR_HSERDY) { // Wait for HSERDY
                	exe()->write_encoded(&dev()->rcc->CR, RCC_CR_CSSON_Msk, RCC_CR_CSSON);
                    rdy = 0;
                }
                else {
                	timeout--;
            ...
            case 2: // HSEBYP: HSE clock bypass
            	exe()->write_encoded(&dev()->rcc->CR, RCC_CR_HSEBYP_Msk, RCC_CR_HSEBYP);
...
void STM32FXXX_Rcc_HSelect(uint8_t hclock)
{
	uint32_t timeout = 5000;
		switch(hclock){
			case RCC_HCLK_HSI: // HSI selected as system clock
				exe()->write_block(&dev()->rcc->CFGR, 2, RCC_CFGR_SW_Pos, 0);
				break;

			case RCC_HCLK_HSE: // HSE oscillator selected as system clock
				exe()->write_block(&dev()->rcc->CFGR, 2, RCC_CFGR_SW_Pos, 1);
				break;

			case RCC_HCLK_PLL:
					exe()->write_block(&dev()->rcc->CFGR, 2, RCC_CFGR_SW_Pos, 2);
				break;
	...
	while((exe()->get_field(dev()->rcc->CFGR, RCC_CFGR_SWS_Msk, RCC_CFGR_SWS_Pos) != hclock) && timeout){timeout--;}
}
uint8_t STM32FXXX_Rcc_PLL_Select(uint8_t hclock)
{ // This bit can be written only when PLL and PLLI2S are disabled
	exe()->write_block(&dev()->rcc->CR, 1, RCC_CR_PLLON_Pos, 0);
	while(exe()->get_block(dev()->rcc->CR, 1, RCC_CR_PLLRDY_Pos));

	exe()->write_block(&dev()->rcc->CR, 1, RCC_CR_PLLI2SON_Pos, 0);
	while(exe()->get_block(dev()->rcc->CR, 1, RCC_CR_PLLI2SRDY_Pos));

	switch(hclock){
		case RCC_CLK_HSI: // HSI
			exe()->write_block(&dev()->rcc->PLLCFGR, 1, RCC_PLLCFGR_PLLSRC_Pos, 0);
		break;
		case RCC_CLK_HSE: // HSE
			exe()->write_block(&dev()->rcc->PLLCFGR, 1, RCC_PLLCFGR_PLLSRC_Pos, 1);
		break;
		default: // HSI
			exe()->write_block(&dev()->rcc->PLLCFGR, 1, RCC_PLLCFGR_PLLSRC_Pos, 0);
		break;
	}
	return exe()->get_block(dev()->rcc->PLLCFGR, 1, RCC_PLLCFGR_PLLSRC_Pos);
}
...
void STM32FXXX_Rcc_LSelect(uint8_t lclock)
{
	...
	switch(lclock)
	{
		case RTC_CLK_LSE: // LSE oscillator clock used as the RTC clock
			exe()->write_block(&dev()->rcc->BDCR, 2, RCC_BDCR_RTCSEL_Pos, 1);
			break;

		case RTC_CLK_LSI: // LSI oscillator clock used as the RTC clock
			exe()->write_block(&dev()->rcc->BDCR, 2, RCC_BDCR_RTCSEL_Pos, 2);
			break;

		case RTC_CLK_HSE: // HSE oscillator clock divided by a programmable prescaler
			exe()->write_block(&dev()->rcc->BDCR, 2, RCC_BDCR_RTCSEL_Pos, 3);
			break;
...
void _STM32FXXX_ppre2(uint8_t ppre2)
{
	switch(ppre2){ // 13
		case 2:
			exe()->write_block(&dev()->rcc->CFGR, 3, RCC_CFGR_PPRE2_Pos, 4);
		break;
		case 4:
			exe()->write_block(&dev()->rcc->CFGR, 3, RCC_CFGR_PPRE2_Pos, 5);
		break;
		case 8:
			exe()->write_block(&dev()->rcc->CFGR, 3, RCC_CFGR_PPRE2_Pos, 6);
		break;
		case 16:
			exe()->write_block(&dev()->rcc->CFGR, 3, RCC_CFGR_PPRE2_Pos, 7);
		break;
		default:
			exe()->write_block(&dev()->rcc->CFGR, 3, RCC_CFGR_PPRE2_Pos, 0);
		break;
	}
}
void _STM32FXXX_ppre1(uint8_t ppre1)
{
	switch(ppre1){ // 10
		case 2:
			exe()->write_block(&dev()->rcc->CFGR, 3, RCC_CFGR_PPRE1_Pos, 4);
		break;
		case 4:
			exe()->write_block(&dev()->rcc->CFGR, 3, RCC_CFGR_PPRE1_Pos, 5);
		break;
		case 8:
			exe()->write_block(&dev()->rcc->CFGR, 3, RCC_CFGR_PPRE1_Pos, 6);
		break;
		case 16:
			exe()->write_block(&dev()->rcc->CFGR, 3, RCC_CFGR_PPRE1_Pos, 7);
		break;
		default:
			exe()->write_block(&dev()->rcc->CFGR, 3, RCC_CFGR_PPRE1_Pos, 0);
		break;
	}
}
void _STM32FXXX_ahbpre(uint16_t ahbpre)
{
	switch(ahbpre){ // 4
		case 2:
			exe()->write_block(&dev()->rcc->CFGR, 4, RCC_CFGR_HPRE_Pos, 8);
		break;
		case 4:
			exe()->write_block(&dev()->rcc->CFGR, 4, RCC_CFGR_HPRE_Pos, 9);
		break;
		case 8:
			exe()->write_block(&dev()->rcc->CFGR, 4, RCC_CFGR_HPRE_Pos, 10);
		break;
		case 16:
			exe()->write_block(&dev()->rcc->CFGR, 4, RCC_CFGR_HPRE_Pos, 11);
		break;
		case 64:
			exe()->write_block(&dev()->rcc->CFGR, 4, RCC_CFGR_HPRE_Pos, 12);
		break;
		case 128:
			exe()->write_block(&dev()->rcc->CFGR, 4, RCC_CFGR_HPRE_Pos, 13);
		break;
		case 256:
			exe()->write_block(&dev()->rcc->CFGR, 4, RCC_CFGR_HPRE_Pos, 14);
			break;
		case 512:
			exe()->write_block(&dev()->rcc->CFGR, 4, RCC_CFGR_HPRE_Pos, 15);
			break;
		default:
			exe()->write_block(&dev()->rcc->CFGR, 4, RCC_CFGR_HPRE_Pos, 0);
		break;
	}
}
void STM32FXXX_Prescaler(uint16_t ahbpre, uint8_t ppre1, uint8_t ppre2, uint8_t rtcpre)
{
	exe()->write_block(&dev()->rcc->CFGR, 5, RCC_CFGR_RTCPRE_Pos, rtcpre);
...
void _STM32FXXX_pllp(uint8_t pllp)
{
	switch(pllp){
			case 2:
				exe()->write_block(&dev()->rcc->PLLCFGR,2,RCC_PLLCFGR_PLLP_Pos,0);
			break;
			case 4:
				exe()->write_block(&dev()->rcc->PLLCFGR,2,RCC_PLLCFGR_PLLP_Pos,1);
			break;
			case 6:
				exe()->write_block(&dev()->rcc->PLLCFGR,2,RCC_PLLCFGR_PLLP_Pos,2);
			break;
			case 8:
				exe()->write_block(&dev()->rcc->PLLCFGR,2,RCC_PLLCFGR_PLLP_Pos,3);
			break;
			default: // 2
				exe()->write_block(&dev()->rcc->PLLCFGR,2,RCC_PLLCFGR_PLLP_Pos,0);
			break;
...
void STM32FXXX_PLL_Division(uint8_t pllm, uint16_t plln, uint8_t pllp, uint8_t pllq)
{
	...
	exe()->write_block(&dev()->rcc->PLLCFGR,4,RCC_PLLCFGR_PLLQ_Pos,pllq);

	_STM32FXXX_pllp(pllp);

	exe()->write_block(&dev()->rcc->PLLCFGR,9,RCC_PLLCFGR_PLLN_Pos,plln);
	exe()->write_block(&dev()->rcc->PLLCFGR,6,RCC_PLLCFGR_PLLM_Pos,pllm);
}
/*** RCC Bit Mapping Definition ***/
/*** Other ***/
void STM32FXXX_RCC_nvic(uint8_t state)
{
	if(state){ exe()->write_bit_block(dev()->core->nvic->ISER, 1, RCC_IRQn, 1); } else{ exe()->write_bit_block(dev()->core->nvic->ICER, 1, RCC_IRQn, 1); }
}
/*** Extended ***/
