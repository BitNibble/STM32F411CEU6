...
static inline void st7789_cs(ST7789_par* par, uint8_t val) {
	if(val) set_pin(par->scl_gpio, par->cs_pin);
	else    clear_pin(par->scl_gpio, par->cs_pin);
}

static inline void st7789_dc(ST7789_par* par, uint8_t val) {
	if(val) set_pin(par->scl_gpio, par->dc_pin);
	else    clear_pin(par->scl_gpio, par->dc_pin);
}

static inline void st7789_rst(ST7789_par* par, uint8_t val) {
	if(val) set_pin(par->scl_gpio, par->rst_pin);
	else    clear_pin(par->scl_gpio, par->rst_pin);
}

// Reset the display
static inline void st7789_reset(ST7789_par* par) {
	st7789_rst(par, 0); _delay_ms(10); st7789_rst(par, 1);
	_delay_ms(120);
}

static inline void st7789_start(ST7789_par* par) {
	clear_pin(par->scl_gpio, par->cs_pin);
}

static inline void st7789_stop(ST7789_par* par) {
...
static inline void st7789_spi_tx(ST7789_par* par, uint8_t d)
{
    ...
    _spi_wait_txe(spi, 10000);

    *(__IO uint8_t*)&spi->DR = d;

    _spi_wait_bsy(spi, 10000);
}
...
void st7789_draw_bits(ST7789_par* par, uint8_t byte, uint16_t fg, uint16_t bg){
	U_word FG = { .var = fg };
	U_word BG = { .var = bg };
	U_byte BYTE = { .var = byte };
	if(BYTE.par.bit7){st7789_data(par, FG.par.h);st7789_data(par, FG.par.l);}else{st7789_data(par, BG.par.h);st7789_data(par, BG.par.l);}
	if(BYTE.par.bit6){st7789_data(par, FG.par.h);st7789_data(par, FG.par.l);}else{st7789_data(par, BG.par.h);st7789_data(par, BG.par.l);}
	if(BYTE.par.bit5){st7789_data(par, FG.par.h);st7789_data(par, FG.par.l);}else{st7789_data(par, BG.par.h);st7789_data(par, BG.par.l);}
	if(BYTE.par.bit4){st7789_data(par, FG.par.h);st7789_data(par, FG.par.l);}else{st7789_data(par, BG.par.h);st7789_data(par, BG.par.l);}
	if(BYTE.par.bit3){st7789_data(par, FG.par.h);st7789_data(par, FG.par.l);}else{st7789_data(par, BG.par.h);st7789_data(par, BG.par.l);}
	if(BYTE.par.bit2){st7789_data(par, FG.par.h);st7789_data(par, FG.par.l);}else{st7789_data(par, BG.par.h);st7789_data(par, BG.par.l);}
	if(BYTE.par.bit1){st7789_data(par, FG.par.h);st7789_data(par, FG.par.l);}else{st7789_data(par, BG.par.h);st7789_data(par, BG.par.l);}
	if(BYTE.par.bit0){st7789_data(par, FG.par.h);st7789_data(par, FG.par.l);}else{st7789_data(par, BG.par.h);st7789_data(par, BG.par.l);}
}
...
void st7789_drawfont8x12(ST7789_par* par, char c, uint16_t x, uint16_t y, uint16_t fg, uint16_t bg)
{
	...
	{
		uint8_t bits = pgm_read_byte(&font8x12[base + row]); // <-- must use &font8x12[index]!
		st7789_draw_bits(par, bits, fg, bg);
	}
}
...
void st7789_drawstring8x12(ST7789_par* par, const char* str, uint16_t x, uint16_t y, uint16_t fg, uint16_t bg)
{
	...
	while (*str) {
		...
		// Advance cursor
		cursorX += 8;  // width of font8x12

		// Optional: wrap if it goes off the edge
		if (cursorX + 8 > par->width) {
			cursorX = 0;
			cursorY += 12; // height of font8x12
		}

		// Optional: stop if we go off the bottom
		if (cursorY + 12 > par->height) break;
	}
}

void st7789_drawfont16x16(ST7789_par* par, char c, uint16_t x, uint16_t y, uint16_t fg, uint16_t bg)
{
	...
	for(uint8_t row = 0; row < glyph_height; row++) {
		U_word bits = { .var = pgm_read_word(&font16x16[base + row]) };
		st7789_draw_bits(par, bits.par.h, fg, bg);
		st7789_draw_bits(par, bits.par.l, fg, bg);
	}
}
...
void st7789_drawstring12x16(ST7789_par* par, const char* str, uint16_t x, uint16_t y, uint16_t fg, uint16_t bg)
{
	...
	while(*str) {
		char c = *str++;
		st7789_drawfont16x16(par, c, cursorX, y, fg, bg);
		cursorX += 12; // fixed spacing
		if(cursorX + 12 > par->width) break;
	}
}

void st7789_drawfont24x24( ST7789_par* par, char c, uint16_t x, uint16_t y, uint16_t fg, uint16_t bg ){
	const uint8_t first = 32;
	...
	for(uint8_t row = 0; row < h; row++) {
		uint8_t b2 = pgm_read_byte(&font24x24[base + row*3 + 0]);
		uint8_t b1 = pgm_read_byte(&font24x24[base + row*3 + 1]);
		uint8_t b0 = pgm_read_byte(&font24x24[base + row*3 + 2]);

		st7789_draw_bits(par, b2, fg, bg);
		st7789_draw_bits(par, b1, fg, bg);
		st7789_draw_bits(par, b0, fg, bg);
	}
}
...
void st7789_drawstring16x24( ST7789_par* par, const char* str, uint16_t x, uint16_t y, uint16_t fg, uint16_t bg ){
	...
	while(*str){
		char c = *str++;
		st7789_drawfont24x24(par, c, cursorX, y, fg, bg);
		cursorX += 16;
...
uint8_t step_bits(uint16_t pixel_pos){
	uint8_t val = 0;
	switch(pixel_pos & 7){
		case 7: val = 0; break;
		case 6: val = 1; break;
...
ST7789 st7789_enable(SPI_TypeDef* spi, uint8_t cs_pin, uint8_t dc_pin, uint8_t rst_pin, uint16_t *fb) {
    ...
    if (spi == SPI1) {
        ...
        st.par.scl_gpio = GPIOB;  // SCK + CS/DC/RST
        st.par.sda_pin  = 5;      // PB5 = SPI1_MOSI
        st.par.scl_pin  = 3;      // PB3 = SPI1_SCK
    } else if (spi == SPI2) {
    ...
    // -------------------------
    st.start           = st7789_start;
    st.stop            = st7789_stop;
    st.spi_tx          = st7789_spi_tx;
    st.cmd             = st7789_cmd;
    st.data            = st7789_data;
    ...
    st.drawfont16x16   = st7789_drawfont16x16;
    st.drawstring12x16 = st7789_drawstring12x16;
    st.drawfont24x24   = st7789_drawfont24x24;
