...
static int ftdelayCycles(uint16_t lock_ID, unsigned int n_cycle, void (*init)(void), void (*term)(void));
static void ftdelayReset(uint16_t ID);
static void ftdelayTerm(uint16_t ID);
